...
class Bot(object):
	...
	def check_time_plugins(self, now=None):
		now = now or datetime.datetime.now()
		for plugin in self._plugins.get_time_plugins(now):
			logger.info('Calling %s since time %s matches', plugin, now)
			plugin(self._client, now)

	def _keep_active(self):
		logger.info('keep active thread started')
		while True:
			self.check_time_plugins()
			time.sleep(60)
			self._client.ping()
...
class PluginsManager(object):
	...
	def get_time_plugins(self, time_now):
		for time_matcher in self.commands['at_time_daily']:
			if time_matcher(time_now):
				yield self.commands['at_time_daily'][time_matcher]
...
class DailyTimeMatcher():
	def __init__(self, hour, minute, second=0):
		self.set_time(hour, minute, second)

	def set_time(self, hour, minute, second=0, day_delta=0):
		dt = datetime.datetime.now() + datetime.timedelta(days=day_delta)
		self.next_time = dt.replace(hour=hour, minute=minute, second=second)

	def __call__(self, dt):
		if dt >= self.next_time:
			self.set_time(self.next_time.hour, self.next_time.minute, self.next_time.second, day_delta=1)
			logger.info("Next calling at %s", self.next_time)
			return True
		else:
			return False

def at_time_daily(hour, minute=0, second=0):
	def wrapper(func):
		PluginsManager.commands['at_time_daily'][DailyTimeMatcher(hour, minute, second)] = func
		logger.info(
			'registered at_time_daily plugin "%s" to "%d:%d:%d"', func.__name__, hour, minute, second)
		return func
